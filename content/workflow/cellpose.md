...
| [cellpose v 0.1.5](https://www.napari-hub.org/plugins/cellpose-napari) | Mandatory  | Install this plugin from within the napari viewer, by going to the **Plugin** menu, then clicking on **Install/uninstall plugins**. Search for *Cellpose-napari* and click **install**. | |
...
Here are a few recommendations before you watch the video:
* Use full screen view  
* Slow the video down by clicking the gear wheel or cog wheel in the lower right corner of the video. Click speed and select a slower speed.  
* Review the parameter setting explanations [here](.\cellpose-parameters.md). In the video the following parameters and features are used:
   * In napari:
     * contrast limits  
     * loading a plugin  
   * In the cellpose-napari plugin:
     * model type
     * channel to segment
     * diameter
     * computer diameter from image
     * average 4 nets
     * resample dynamics
     ...
     * run segmentation
     * cellprob threshold
     * recompute last masks with new cellprob + model match
     :::{hint}
     When any of the buttons are clicked, it may take a few seconds for napari to respond with the new information.  Please be patient. 
     :::
